...
public:
	...
	void Level(Node<T> * root, int level, std::stringstream &aux) const; 
	int altura()const;
	uint leaves() const;
	bool isFull() const;
	T ancestor(T) const;
...
template <class T>
void BST<T>::Level(Node<T> * root, int level, std::stringstream &aux) const{
	if (root == NULL)
	return; 
